...
const Navbar = () => {
  ...
  return (
    ...
    <nav className="bg-[#FFFAF5] py-[21px] shadow-xl  fixed top-0 w-full z-50">
      <div className="container mx-auto flex justify-between items-center lg:flex-row flex-row-reverse">
        ...
        <Link href="/">
          <div className="flex justify-center items-center">
            <Image alt="pic" src={logo} width={100} />
          </div>
        </Link>
        ...
        <div className="hidden lg:block">
          <ul className="flex text-textPrimary font-inter font-bold leading-none text-fluid-base tracking-wide capitalize space-x-[17px] p-[10px]">
            {menuItems.map((item) => (
              <Link href={item?.path} key={item.name} className={`${pathName==item.path ? "text-[#F88D58]" :""} border-b-2 py-2 border-transparent hover:border-[#F88D58] duration-300`}>
